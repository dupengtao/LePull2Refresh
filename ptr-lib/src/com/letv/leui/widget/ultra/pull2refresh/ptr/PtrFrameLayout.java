...
public class PtrFrameLayout extends FrameLayout {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.LePtrFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.LePtrFrameLayout_le_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.LePtrFrameLayout_le_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.LePtrFrameLayout_le_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.LePtrFrameLayout_le_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.LePtrFrameLayout_le_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.LePtrFrameLayout_le_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.LePtrFrameLayout_le_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.LePtrFrameLayout_le_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            ...
            LogHelper.e(LOG_TAG, "run --- !mScroller.computeScrollOffset()=" + !mScroller.computeScrollOffset() + "--- mScroller.isFinished()=" + mScroller.isFinished());
            ...
            if (!finish) {
                ...
                if(deltaY!=0) {
                    LogHelper.e("tryToScrollTo", "run -- deltaY = " + deltaY + "");
                }
                mLastFlingY = curY;
                movePos(deltaY);
                post(this);
